...
export default function ServiceCardAdmin({data, index}) {
  ...
  const deleteFolder = () => {
    router.push(`/dashboard/services/delete/${data.id}`);
  };
  ...
  return (
    <>
      <Link
        ...
        href={`${process.env.NEXT_PUBLIC_BASE_URL}/dashboard/services/${data.id}`}
      >
        <div className="bg-slate-50 hover:bg-slate-100 rounded-md shadow-md p-5 flex items-center justify-start gap-3 z-30">
          ...
          <div className="w-full flex items-center justify-between">
            <div className="flex flex-1 flex-col items-start justify-between">
              <h3 className="text-base font-bold uppercase">{data.title}</h3>
